...
-- Source: supabase/migrations/20260730000000_project_templates.sql
-- ============================================================
-- Een organisatie legt haar vaste werkwijze één keer vast als sjabloon en rolt
-- die bij elk nieuw project uit. Datums staan relatief vast (dagoffsets t.o.v.
-- de projectstart) omdat een sjabloon niet aan een kalender hangt.
...
begin
  ...
  loop
    ...
    -- Weekplanner-volgorde: hang de taken achter wat er al op die dag staat, in
    -- sjabloonvolgorde. Stappen van 1000, net als reorder_task_planning.
    ...
    insert into public.tasks (
      ...
      p_organization_id,
      -- tasks.created_by heeft géén default, dus expliciet zetten.
      auth.uid(),
      p_project_id,
      ...
      -- alle uitgerolde taken zouden dezelfde created_at delen en willekeurig op id
      -- gesorteerd worden.
      clock_timestamp()
    );
