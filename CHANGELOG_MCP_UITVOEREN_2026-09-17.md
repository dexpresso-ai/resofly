...
## Samengevoegd met de kerntools van Gerrie

Deze branch is gebouwd naast "de gekoppelde AI kan alles wat Gerrie kan"
(`CHANGELOG_MCP_GERRIE_PARITEIT_2026-09-17.md`) en daarna met staging
samengevoegd. Daarbij kwamen drie dingen samen:

- **Kerntools worden altijd klaargezet.** `find_actions`, `run_action` en
  `propose_action` kennen nu ook Gerrie's kerntools, en `execute_action` ook —
  maar een kerntool heeft geen uitvoerder op de server en valt dus altijd terug
  op een voorstel (`direct: false`). Een factuur, een mail aan een klant of een
  reactie op een ticket gaat nooit zonder klik de deur uit.
- **De migratie heet `20260917050000_mcp_execute.sql`** (was `20260918000000`).
  Een tijdstempel van morgen staat vóór elke migratie die later vandaag nog komt
  in de weg, en botst met de eerste van morgen.
- **De melding houdt de titel van een kerntool-voorstel.** Deze migratie vervangt
  `push_on_mcp_proposal()`, net als `20260917040000`, maar kende diens
  `result->>'title'` niet en zou hem stil ongedaan maken. Die regel staat er nu
  in, en `mcpExecute.test.ts` bewaakt hem.
