...
export type NoteHandwritingPaper = 'blank' | 'lined' | 'dotted' | 'grid';
/** Samenvatting van het handschrift (pen op tablet) bij een notitie, zónder de zware `pages`-kolom: genoeg voor een badge "handschrift · 2 pagina's". */
export interface NoteHandwritingSummary extends OrgScopedRow {
  note_id: UUID; page_count: number; stroke_count: number; paper: NoteHandwritingPaper; created_at: string; updated_at: string;
}
/** Het volledige handschrift: `pages` is een inktdocument (src/lib/ink.ts); `parseInkDocument` leest het defensief. */
export interface NoteHandwriting extends NoteHandwritingSummary {
  pages: unknown;
}
