...
export function ProjectPage({ data, project, canWrite, onNewTask, onEditTask, onEditProject, setTaskStatus }: { data: AppData; project: Project; canWrite: boolean; onNewTask: () => void; onEditTask: (task: Task) => void; onEditProject: () => void; setTaskStatus: (task: Task, status: TaskStatus) => void }) {
  const tasks = data.tasks.filter(t => t.project_id === project.id);
  const client = data.clients.find(c => c.id === project.client_id);
  return <><div className="proj-fin-panel"><div className="proj-fin-header"><div><h3>{project.name}</h3><p className="pc-desc">{client?.name ?? 'Geen klant'} · {project.description ?? ''}</p>{project.archived && <span className="status-pill archived">Gearchiveerd</span>}</div><div className="proj-actions"><Button onClick={onEditProject}>Project bewerken</Button><Button variant="primary" onClick={onNewTask} disabled={project.archived || !canWrite}>+ Taak</Button></div></div></div>
