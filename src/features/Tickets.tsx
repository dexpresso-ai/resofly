...
import { ArrowDown, ArrowUp, List, RotateCcw, Search, SlidersHorizontal, Table as TableIcon } from 'lucide-react';
...
type TicketFilters = { query: string; status: string; priority: string; clientId: string };
const emptyTicketFilters: TicketFilters = { query: '', status: '', priority: '', clientId: '' };
...
function filterTickets(data: AppData, filters: TicketFilters): Ticket[] {
...
export function Tickets({ data, onNew, onEdit, onConvert, unreadTicketIds }: { data: AppData; onNew: () => void; onEdit: (t: Ticket) => void; onConvert: (t: Ticket) => void; unreadTicketIds: Set<string> }) {
  ...
  return <>
    ...
    {view === 'list'
      ? <TicketCardList data={data} onEdit={onEdit} onConvert={onConvert} unreadTicketIds={unreadTicketIds} />
      : <TicketTable data={data} filters={filters} onFilters={setFilters} onEdit={onEdit} onConvert={onConvert} unreadTicketIds={unreadTicketIds} />}
  </>;
}
...
function TicketCardList({ data, onEdit, onConvert, unreadTicketIds }: { data: AppData; onEdit: (t: Ticket) => void; onConvert: (t: Ticket) => void; unreadTicketIds: Set<string> }) {
  if (data.tickets.length === 0) return <div className="empty quote-table-empty"><div className="e-big">Nog geen tickets</div><p>Maak je eerste ticket aan om binnenkomende vragen op te volgen.</p></div>;
...
function TicketTable({ data, filters, onFilters, onEdit, onConvert, unreadTicketIds }: { data: AppData; filters: TicketFilters; onFilters: (filters: TicketFilters) => void; onEdit: (t: Ticket) => void; onConvert: (t: Ticket) => void; unreadTicketIds: Set<string> }) {
  const [sort, setSort] = useState<SortState>({ key: 'created', dir: 'desc' });
  const filtered = useMemo(() => filterTickets(data, filters), [data, filters]);
  const sorted = useMemo(() => sortTickets(filtered, data, sort), [filtered, data, sort]);
  ...
  return <>
    <TicketFilterPanel data={data} filters={filters} visibleCount={sorted.length} onChange={onFilters} />

    <div className="quote-table-card ticket-table-card">
      ...
      {data.tickets.length === 0 && <div className="empty quote-table-empty"><div className="e-big">Nog geen tickets</div><p>Maak je eerste ticket aan om binnenkomende vragen op te volgen.</p></div>}
      {data.tickets.length > 0 && sorted.length === 0 && <div className="empty quote-table-empty finance-search-empty"><div className="e-big">Geen tickets gevonden</div><p>Pas je zoekterm of filters aan om meer resultaten te tonen.</p><button type="button" onClick={() => onFilters(emptyTicketFilters)}><RotateCcw size={14}/> Filters wissen</button></div>}
    </div>
  </>;
}
...
function TicketFilterPanel({ data, filters, visibleCount, onChange }: { data: AppData; filters: TicketFilters; visibleCount: number; onChange: (filters: TicketFilters) => void }) {
  ...
  return <section className="finance-search-card ticket-search-card" aria-label="Tickets zoeken en filteren">
    ...
    <div className="finance-search-grid ticket-search-grid">
      ...
      <label className="field finance-search-field"><span>Klant</span><Select className="form-select" value={filters.clientId} onChange={event => update({ clientId: event.target.value })}><option value="">Alle klanten</option>{clientOptions.map(client => <option key={client.id} value={client.id}>{client.name}</option>)}</Select></label>
    </div>
