...
import { CalendarPage, type CalendarDraft } from './features/CalendarPage';
import { TaskAgendaSection } from './components/TaskAgenda';
...
import { applyPeriodLocally, applyPlanningLocally, applyTimeLocally, clockLabel, mergeTaskRows, restoreTasksForDates, splitTitleAndEstimate } from './lib/planning';
...
function App() {
  ...
  /** "Tijd reserveren" vanuit een taak: de agenda opent met dit concept. */
  ...
  /**
   * Zet een taak op een dag én een tijdstip: een blok in het rooster van de
   * weekplanner (en van de agenda). Dezelfde optimistische route als het slepen
   * van een kaart: meteen in beeld, dan de server, en bij een dagwissel alleen
   * de twee geraakte dagen teruglezen voor de volgorde. `minutes` alleen
   * meegeven als de duur verandert — de duur van een blok ís de schatting.
   */
  async function planTaskAtTime(taskId: string, plannedDate: string, startMinute: number, minutes?: number | null) {
    if (!ensureCanWrite()) return;
    setError(null);
    const previousTasks = data.tasks;
    const moving = previousTasks.find(task => task.id === taskId);
    if (!moving) return;
    const fromDate = moving.planned_date ?? null;
    const patched = applyTimeLocally(previousTasks, taskId, plannedDate, startMinute, minutes);
    const local = patched.find(task => task.id === taskId)!;

    setData(prev => ({ ...prev, tasks: applyTimeLocally(prev.tasks, taskId, plannedDate, startMinute, minutes) }));
    try {
      const saved = await updateRow<Task>('tasks', taskId, {
        planned_date: plannedDate,
        planned_end_date: null,
        planned_start_minute: local.planned_start_minute,
        planned_order: local.planned_order,
        ...(minutes === null || minutes === undefined ? {} : { estimated_minutes: local.estimated_minutes }),
      }, activeOrg.id);
      const affected = fromDate && fromDate !== plannedDate
        ? await fetchTasksForPlannedDates(activeOrg.id, [fromDate, plannedDate])
        : [];
      setData(prev => ({ ...prev, tasks: mergeTaskRows(prev.tasks, [saved, ...affected]) }));
    } catch (e) {
      setData(prev => ({ ...prev, tasks: restoreTasksForDates(prev.tasks, previousTasks, [fromDate, plannedDate]) }));
      throw e;
    }
  ...
  async function quickAddTask(plannedDate: string, title: string, plannedEndDate?: string | null, time?: { startMinute: number; minutes: number } | null): Promise<Task | null> {
    ...
    // Behalve bij een getekend tijdvak in het rooster: een blok hééft een duur,
    // en dat is dan het vak dat je tekende (tenzij de titel het anders zegt).
    const parsed = splitTitleAndEstimate(title);
    const spanning = !!plannedEndDate && plannedEndDate > plannedDate;
    const created = await insertRow<Task>('tasks', activeOrg.id, {
      title: parsed.title,
      estimated_minutes: parsed.minutes ?? (time && !spanning ? Math.max(15, Math.round(time.minutes)) : null),
      planned_date: plannedDate,
      planned_end_date: spanning ? plannedEndDate : null,
      planned_start_minute: time && !spanning ? Math.max(0, Math.min(1439, Math.round(time.startMinute))) : null,
    });
  ...
  /**
   * Klant, project en taak van een afspraak (her)koppelen. `taskId` weglaten
   * betekent: de taak die er al aan hangt laten staan — zo raakt een aanroep
   * die alleen de klant wijzigt de taak niet stilletjes kwijt. `null` wist hem.
   */
  async function setCalendarEventLink(event: CalendarExternalEvent, clientId: string | null, projectId: string | null, trackTime: boolean = true, taskId?: string | null) {
    if (!ensureCanWrite()) return;
    setLoading(true); setError(null);
    try {
      const existing = eventLinkFor(event);
      const nextTaskId = taskId === undefined ? existing?.task_id ?? null : taskId;
      if (!clientId && !projectId && !nextTaskId) {
  ...
  /** Een taak aan een bestaande afspraak hangen. Klant en project laat de
   *  database uit de taak volgen; wat er al aan de koppeling hing blijft staan
   *  als de taak zelf geen project of klant heeft. */
  async function linkTaskToEvent(task: Task, event: CalendarExternalEvent) {
    if (event.visibility !== 'organization' || event.is_private_masked) {
      setError('Een privé-afspraak is niet aan een taak te koppelen. Deel de agenda eerst met de organisatie.');
      return;
    }
    const existing = eventLinkFor(event);
    await setCalendarEventLink(event, existing?.client_id ?? task.client_id ?? null, existing?.project_id ?? task.project_id ?? null, existing?.track_time ?? true, task.id);
  }

  /** Alleen de taak van de koppeling af; klant, project en de uren-instelling blijven. */
  async function unlinkTaskEvent(link: CalendarEventLink) {
    if (!ensureCanWrite()) return;
    setLoading(true); setError(null);
    try {
      await setCalendarEventLinkTask(link.id, activeOrg.id, null);
      await refresh();
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Afspraak ontkoppelen mislukt');
    } finally {
      setLoading(false);
    }
  }

  /** Tijd reserveren: naar de agenda op de plandatum, met de taak als
   *  concept-afspraak. De agenda opent het aanmaakpaneel en meldt zich terug
   *  zodra het concept verwerkt is. Duur = wat er van de schatting nog open staat. */
  function reserveTimeForTask(task: Task) {
    if (!ensureCanWrite()) return;
    const project = task.project_id ? data.projects.find(p => p.id === task.project_id) ?? null : null;
    const logged = data.timeEntries.filter(entry => entry.task_id === task.id).reduce((sum, entry) => sum + entry.minutes, 0);
    const estimate = task.estimated_minutes != null && Number.isFinite(Number(task.estimated_minutes)) ? Number(task.estimated_minutes) : 0;
    const date = task.planned_date ?? formatISODate(new Date());
    setCalendarDraft({
      taskId: task.id,
      title: task.title,
      clientId: project?.client_id ?? task.client_id ?? '',
      projectId: task.project_id ?? '',
      minutes: estimate > 0 ? Math.max(30, estimate - logged) : 60,
      date,
    });
    setEdit(null);
    setCalendarJump(date);
    setPage('calendar');
  }
...
/** Afspraak ↔ taak vanuit het taakvenster; de handlers leven in de werkruimte. */
type TaskAgendaHandlers = {
  onReserve: (task: Task) => void;
  onLinkEvent: (task: Task, event: CalendarExternalEvent) => Promise<void>;
  onUnlink: (link: CalendarEventLink) => Promise<void>;
...
function cleanForm(kind: string, form: Record<string, any>, clientFieldDefinitions: ClientFieldDefinition[] = []) {
  ...
  if (kind === 'task') {
    ...
    // Het tijdstip komt als "HH:MM" uit het formulier; de database wil minuten
    // na middernacht — en geen tijd zonder dag of op een weekstrook.
    const clock = typeof cleaned.planned_start_time === 'string' ? cleaned.planned_start_time.match(/^(\d{1,2}):(\d{2})/) : null;
    const minute = clock ? Number(clock[1]) * 60 + Number(clock[2]) : null;
    cleaned.planned_start_minute = minute !== null && Number.isFinite(minute) && minute >= 0 && minute < 1440 && cleaned.planned_date && !cleaned.planned_end_date
      ? minute
      : null;
    delete cleaned.planned_start_time;
  }
